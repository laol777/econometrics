...
MSample <- GetAproxSample(train, M)
trainSample <- GetTrainSample(train, M)

indexSimilar <- GetIndexStartMostSimilarSample(trainSample, MSample, step)

MSample <- GetAproxSample(trainDelta, M)
trainSample <- GetTrainSample(trainDelta, M)

sum(dataDelta[(indexSimilar - 1):(indexSimilar + M - 2)] > 0)
sum(MSample > 0)
sum( (dataDelta[(indexSimilar - 1):(indexSimilar + M - 2)] > 0) & (MSample > 0))
